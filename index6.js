...
class TwoStepProcessor extends AuthProcessor {
  ...
  validate(username, passkey) {
    if (username === "john" && passkey === "password" && this.isValidTwoStepCode()) {
      ...
      // Передаємо запит на аутентифікацію наступному обробнику в ланцюгу.
...
class CredentialsProcessor extends AuthProcessor {
  // Метод для перевірки аутентифікації validate. Перевіряє облікові дані користувача.
...
class ProcessorBuilder {
  constructor() {
    // Властивість, що зберігає перший обробник у ланцюгу. За замовчуванням - null.
    this.firstProcessor = null;
    // Властивість, що зберігає останній обробник у ланцюгу. За замовчуванням - null.
  ...
  add(processor) {
    if (!this.firstProcessor) {
      // Якщо це перший обробник, зберігається як перший і останній.
      this.firstProcessor = processor;
      this.lastProcessor = processor;
    } else {
      // Якщо це не перший обробник, додається в кінець ланцюга і стає останнім.
      this.lastProcessor.setNextProcessor(processor);
      this.lastProcessor = processor;
    }

    // Повертає this, щоб дозволити подальше ланцюжкове викликання.
    return this;
  }
